...
export async function proxy(request: NextRequest) {
  ...
  const publicApis = ["/api/verify-company", "/api/send-magic-link", "/api/send-reset-password", "/api/admin/repas-photo", "/api/admin/exclusions", "/api/chat/send", "/api/profil/poids", "/api/cron/daily-email"]
  ...
  if (pathname === "/" || pathname.startsWith("/login") || pathname.startsWith("/auth") || pathname.startsWith("/email-preview") || pathname.startsWith("/preview-themes") || pathname.startsWith("/preview-checkin")) {
